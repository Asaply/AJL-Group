...
- Políticas en `storage.objects` para `bucket_id = 'task-files'`: SELECT e INSERT si `public.is_partner()`; DELETE si `public.is_partner() AND owner_id = auth.uid()::text` (`owner_id` es la columna vigente de `storage.objects`; `owner` está deprecada).
